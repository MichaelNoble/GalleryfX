<?php
namespace GFXI\JssorHelper;
/**
 * Helpers for slide transition,arrow and bullet skin
 */


function getOptionsData($paired) {

    include(GFXI_PLUGIN_DIR . 'lib/jssor.data.php');

    $result = array();
    if ( strpos($paired,'Transition') === false ) {
        foreach (${$paired} as $key => $data)
        {
            $result[] = array('value' => $key, 'label' => $data );
        }
    } else {
        foreach (${$paired} as $key => $data)
        {
            $result[] = array('value' => $data, 'label' => $key );
        }      
    }
 ...
 * @return string
 */
function getslide_trans( $slides ,$default ) {
	
	$string = '';
	$def_flag = false;
	for( $flag = 0; $flag < count($slides); $flag++ ) {
		$trans = ( isset($slides[$flag]['transits'][0]['slide-trans']) ? $slides[$flag]['transits'][0]['slide-trans'] : null );
		if ( ( $trans == 'NULL' ) || ( $trans == NULL ) ) {
			$def_flag = true;
			continue;
		}
		if ( strpos( $string, $trans) > 0 ) continue; 
		$string .= $trans . ",\n";
	}
	
	if ($def_flag) $string .= $default . ",\n";
 ...
 * @return string
 */
function getcaption_trans( $slides, $result, $string_id, $default ) {
 
	$string = '';
	foreach ( $slides as $slide ) {
		$trans = (isset($slide['transits'][0][$string_id]) ? $slide['transits'][0][$string_id] : null );
		if ( ( $trans == 'NULL' ) || ( $trans == NULL ) ) {
			$trans = $default;
		}
		if ( strpos( $string, $trans) > 0 ) continue; 
...
function bulletSkin( $bullet ) {
	...
	switch( $bullet ){
		...
		case 'b02' :
			$style1 = '.jssorb02 div,.jssorb02 div:hover,.jssorb02 .av { background:url( ' . GFXI_PLUGIN_URL . 'components/jssor/img/b02.png ) no-repeat; overflow:hidden; cursor:pointer; }  .jssorb02 div { background-position:-5px -5px; }  .jssorb02 div:hover,.jssorb02 .av:hover { background-position:-35px -5px; }  .jssorb02 .av { background-position:-65px -5px; }  .jssorb02 .dn,.jssorb02 .dn:hover { background-position:-95px -5px; }';
			...
			return array( $style1, $style2, $NT );
		case 'b03' :
			$style1 = '.jssorb03 div,.jssorb03 div:hover,.jssorb03 .av { background:url( ' . GFXI_PLUGIN_URL . 'components/jssor/img/b03.png ) no-repeat; overflow:hidden; cursor:pointer; }  .jssorb03 div { background-position:-5px -4px; }  .jssorb03 div:hover,.jssorb03 .av:hover { background-position:-35px -4px; }  .jssorb03 .av { background-position:-65px -4px; }  .jssorb03 .dn,.jssorb03 .dn:hover { background-position:-95px -4px; }';
			$style2	= 'position: absolute; width: 21px; height: 21px; text-align:center; line-height:21px; color:white; font-size:12px;';			
			$NT = '<numbertemplate></numbertemplate>';
			return array( $style1, $style2, $NT );	
		case 'b04' :
			$style1 = '.jssorb04 div,.jssorb04 div:hover,.jssorb04 .av { background:url( ' . GFXI_PLUGIN_URL . 'components/jssor/img/b04.png ) no-repeat; overflow:hidden; cursor:pointer; }  .jssorb04 div { background-position:-5px -5px; }  .jssorb04 div:hover,.jssorb04 .av:hover { background-position:-35px -5px; }  .jssorb04 .av { background-position:-65px -5px; }  .jssorb04 .dn,.jssorb04 .dn:hover { background-position:-95px -5px; }';
			$style2	= 'position: absolute; width: 19px; height: 19px; text-align:center; line-height:19px; color:White; font-size:12px;';			
			$NT = NULL;
			return array( $style1, $style2, $NT );
		case 'b05' :
			$style1 = '.jssorb05 div,.jssorb05 div:hover,.jssorb05 .av { background:url( ' . GFXI_PLUGIN_URL . 'components/jssor/img/b05.png ) no-repeat; overflow:hidden; cursor:pointer; }  .jssorb05 div { background-position:-7px -7px; }  .jssorb05 div:hover,.jssorb05 .av:hover { background-position:-37px -7px; }  .jssorb05 .av { background-position:-67px -7px; }  .jssorb05 .dn,.jssorb05 .dn:hover { background-position:-97px -7px; }';
			$style2 = 'position: absolute; width: 16px; height: 16px;';	
			$NT = NULL;
			return array( $style1, $style2, $NT );
		case 'b06' :
			$style1 = '.jssorb06 div,.jssorb06 div:hover,.jssorb06 .av { background:url( ' . GFXI_PLUGIN_URL . 'components/jssor/img/b06.png ) no-repeat; overflow:hidden; cursor:pointer; }  .jssorb06 div { background-position:-6px -6px; }  .jssorb06 div:hover,.jssorb06 .av:hover { background-position:-36px -6px; }  .jssorb06 .av { background-position:-66px -6px; }  .jssorb06 .dn,.jssorb06 .dn:hover { background-position:-96px -6px; }';
			$style2 = 'position: absolute; width: 18px; height: 18px;';	
			$NT = NULL;
			return array( $style1, $style2, $NT );
		case 'b07' :
			$style1 = '.jssorb07 div,.jssorb07 div:hover,.jssorb07 .av { background:url( ' . GFXI_PLUGIN_URL . 'components/jssor/img/b07.png ) no-repeat; overflow:hidden; cursor:pointer; }  .jssorb07 div { background-position:-5px -5px; }  .jssorb07 div:hover,.jssorb07 .av:hover { background-position:-35px -5px; }  .jssorb07 .av { background-position:-65px -5px; }  .jssorb07 .dn,.jssorb07 .dn:hover { background-position:-95px -5px; }';
			$style2 = 'position: absolute; width: 20px; height: 20px;';	
			$NT = NULL;
			return array( $style1, $style2, $NT );
		case 'b08' :
			$style1 = '.jssorb08 div,.jssorb08 div:hover,.jssorb08 .av { background:url( ' . GFXI_PLUGIN_URL . 'components/jssor/img/b08.png ) no-repeat; overflow:hidden; cursor:pointer; }  .jssorb08 div { background-position:-5px -5px; }  .jssorb08 div:hover,.jssorb08 .av:hover { background-position:-35px -5px; }  .jssorb08 .av { background-position:-65px -5px; }  .jssorb08 .dn,.jssorb08 .dn:hover { background-position:-95px -5px; }';
			$style2 = 'position: absolute; width: 19px; height: 19px; text-align:center; line-height:19px; color:White; font-size:12px;';	
			$NT = '<NumberTemplate></NumberTemplate>';
			return array( $style1, $style2, $NT );
		case 'b09' :
